...
def save_fault_edit(fault_id, name, causes, actions):
    ...
    for c in custom_list:
        if c["id"] == fault_id:
            ...
            rebuild_faults()
            return
    data.setdefault("overrides", {})[fault_id] = {"name": name, "causes": causes, "actions": actions}
    _save_overrides(data)
    rebuild_faults()


def add_custom_fault(name, technique, causes, actions):
    ...
    while fid in existing_ids:
        ...
        n += 1
    entry = dict(id=fid, technique=technique, name=name, tags=[], causes=causes, actions=actions)
...
class WizardScreen(Screen):
    ...
    def build(self):
        ...
        diag_btn = Button(text="Diagnose", background_color=AMBER)
...
class SpectrumScreen(Screen):
    ...
    def build(self):
        ...
        for t in self.toggle_tags:
            b = Button(text=self.toggle_labels[t], size_hint_y=None, height=dp(40))
        ...
        btn = Button(text="Diagnose", size_hint_y=None, height=dp(48), background_color=AMBER)
    ...
    def _num_field(self, col, label):
        ...
        ti = TextInput(text="0", multiline=False, input_filter="float",
                        size_hint_y=None, height=dp(40))
        col.add_widget(ti)
        return ti
    ...
    def make_flip(self, tag):
        def _flip(instance):
            ...
            instance.background_color = AMBER if self.toggle_state[tag] else PANEL
        return _flip
...
class LibraryScreen(Screen):
    ...
    def build(self):
        ...
        search_row = BoxLayout(size_hint_y=None, height=dp(44), padding=dp(6), spacing=dp(6))
        self.search = TextInput(hint_text="Search faults...", multiline=False)
        self.search.bind(text=self.refresh)
        search_row.add_widget(self.search)
        add_btn = Button(text="+ Add", size_hint_x=None, width=dp(80), background_color=AMBER)
    ...
    def refresh(self, *a):
        ...
        for f in FAULTS:
            if query and query not in f["name"].lower():
                continue
            btn = Button(text="%s  [%s]" % (f["name"], f["technique"]),
                         size_hint_y=None, height=dp(44), background_color=PANEL,
                         halign="left")
    ...
    def open_form(self, fault=None):
        ...
        name_in = TextInput(text=fault["name"] if is_edit else "", multiline=False,
                             size_hint_y=None, height=dp(40))
        ...
        actions_in = TextInput(text="\n".join(fault["actions"]) if is_edit else "",
                                multiline=True, size_hint_y=None, height=dp(150))
        inner.add_widget(actions_in)

        if not is_edit:
            inner.add_widget(body_label(
                "Note: new entries are searchable here in the Library, but won't "
                "auto-match in the Wizard's spectrum scoring - that needs signature "
                "tags wired into the code.", MUTED))

        sc.add_widget(inner)
        content.add_widget(sc)

        btn_row = BoxLayout(size_hint_y=None, height=dp(44), spacing=dp(6))
        save_btn = Button(text="Save", background_color=TEAL)
        ...
        def _save(*a):
            ...
            if is_edit:
                save_fault_edit(fault["id"], name, causes, actions)
            else:
                add_custom_fault(name, tech_spinner.text, causes, actions)
            popup.dismiss()
            self.refresh()
...
class SeverityScreen(Screen):
    ...
    def build(self):
        ...
        self.rms = TextInput(text="2.8", multiline=False, input_filter="float",
                              size_hint_y=None, height=dp(40))
        ...
        btn = Button(text="Calculate", size_hint_y=None, height=dp(44), background_color=AMBER)
...
class ReportScreen(Screen):
    ...
    def build(self):
        ...
        self.tag = TextInput(multiline=False, size_hint_y=None, height=dp(40))
        ...
        self.bearing_pos = TextInput(text="NDE", multiline=False, size_hint_y=None, height=dp(40))
        ...
        self.overall_vib = TextInput(multiline=False, input_filter="float", size_hint_y=None, height=dp(40))
        col.add_widget(self.overall_vib)

        col.add_widget(section_label("Dominant frequency (Hz)", MUTED))
        self.dom_freq = TextInput(multiline=False, input_filter="float", size_hint_y=None, height=dp(40))
        ...
        self.tech_note = TextInput(multiline=True, size_hint_y=None, height=dp(60))
        col.add_widget(self.tech_note)

        col.add_widget(section_label("Physical inspection", MUTED))
        self.sound_buttons = {}
        for s in self.SOUND_SOURCES:
            b = Button(text="Sound: %s" % s, size_hint_y=None, height=dp(38), background_color=PANEL)
            b.bind(on_release=self.make_sound_flip(s))
            self.sound_buttons[s] = b
            col.add_widget(b)
        self.struct_btn = Button(text="Structural damage at base", size_hint_y=None,
                                  height=dp(38), background_color=PANEL)
        ...
        self.extra_note = TextInput(multiline=True, size_hint_y=None, height=dp(60))
        ...
        gen_btn = Button(text="Generate report", size_hint_y=None, height=dp(46), background_color=AMBER)
        gen_btn.bind(on_release=self.generate)
        col.add_widget(gen_btn)

        col.add_widget(section_label("Report preview", MUTED))
        self.output = TextInput(text="", readonly=True, size_hint_y=None, height=dp(300))
        col.add_widget(self.output)

        copy_btn = Button(text="Copy report", size_hint_y=None, height=dp(44))
    ...
    def make_sound_flip(self, s):
        def _flip(instance):
            ...
            instance.background_color = AMBER if self.sound_state[s] else PANEL
        return _flip

    def flip_structural(self, instance):
        self.structural_damage = not self.structural_damage
        instance.background_color = AMBER if self.structural_damage else PANEL
...
class RootWidget(BoxLayout):
    def __init__(self, **kw):
        ...
        nav = BoxLayout(size_hint_y=None, height=dp(46), spacing=dp(2))
        ...
        for label, screen_name in tabs:
            b = Button(text=label, background_color=PANEL)
            b.bind(on_release=self.make_switch(screen_name))
            self.nav_buttons[screen_name] = b
            nav.add_widget(b)
        self.add_widget(nav)
    ...
    def highlight(self, screen_name):
        for name, btn in self.nav_buttons.items():
            btn.background_color = AMBER if name == screen_name else PANEL
